...
correct_prediction = tf.equal(tf.argmax(emotion,1),tf.argmax(selfassess(),1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction,tf.float32))
...
for k in range(2,5):
     ...
     sqlday = "select open_price from day_price,symbol where day_price.symbol=symbol.symbol and symbol.id="
     sqlday += str(k)
     sqlday += " ORDER BY day_price.price_date desc  limit 356"
     # cursor.execute(sql)
     # stockdata = cursor.fetchall()
     # data = cursor.fetchone()
     df = pd.read_sql(sql=sqlday, con=conn)
     stock = preprocessing.minmax_scale(df,feature_range=(-1,1))
     #new_cloass = addLayer(stock / firstmind, 1, 1, n_layer=1, activation_function=tf.nn.tanh)
     # newconscuous = addLayer(firstmind * moveloss * new_cloass, input_size, 1, n_layer=1,activation_function=tf.nn.tanh)
     # reinforceconscious = addLayer(firstmind / (reward - emotion), input_size, 1, n_layer=1,activation_function=tf.nn.tanh)
     ...
     for i in range(3200):
          ...
          loss_wisdom, traina_wisdom = sess.run([losswisdow, trainwisdow], feed_dict={xs: reward, ys: moveloss})
          acc = sess.run(accuracy, feed_dict={xs: moveloss, ys: stock})
          ...
          if i % 3000 == 0:
             ...
             print("accuracy", sess.run(accuracy, feed_dict={xs: moveloss, ys: stock}))
...
with tf.name_scope('loss'):
    ...
    tf.summary.scalar('loss_emotion', loss)
    ...
    loss_position = tf.reduce_mean(tf.reduce_sum(tf.square((moveloss - environment())), reduction_indices=[1]))
    tf.summary.scalar('loss_position', loss_position)

with tf.name_scope('accuracy'):
    correct_prediction_board = tf.equal(tf.argmax(emotion, 1), tf.argmax(selfassess(), 1))
    accuracy_board = tf.reduce_mean(tf.cast(correct_prediction_board, tf.float32))
    tf.summary.scalar('accuracy', accuracy_board)
...
with tf.name_scope('train'):
        ...
        train_step3 = tf.train.GradientDescentOptimizer(lr).minimize(loss_position)
        accuracy_board = tf.reduce_mean(tf.cast(correct_prediction_board, tf.float32))
...
for i in range(10000):
    ...
    if i % 50 == 0:
        ...
        result4 = sess.run(merged, feed_dict={xs: moveloss, ys: stock})
        writer.add_summary(result4, i)
